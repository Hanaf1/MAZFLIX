...
class _StockInputScreenState extends State<StockInputScreen> {
  ...
  Future<void> _saveStockData(String productId) async {
    ...
    // Get today's stock and restock values
    final stok = int.tryParse(inputData[productId]?['stok'] ?? '0') ?? 0;
    final restock = int.tryParse(inputData[productId]?['restock'] ?? '0') ?? 0;
    final product = products[productId];

    if (product != null) {
      // Get yesterday's data from Firebase
      final yesterdayData = await _database.child('stok_harian/$yesterday/$productId').get();

      // Extract yesterday's stock and restock (purchase) values
      final yesterdayStok = yesterdayData.exists ? (yesterdayData.value as Map)['stok'] ?? 0 : 0;
      final yesterdayRestock = yesterdayData.exists ? (yesterdayData.value as Map)['beli'] ?? 0 : 0;

      // Calculate sales using the correct formula
      final laku = (yesterdayStok + yesterdayRestock) - stok;

      final int hargaBeli = product['hargaBeli'] ?? 0;
      final int hargaJual = product['hargaJual'] ?? 0;

      // Calculate financial values based on actual sales (laku)
      final int modal = laku * hargaBeli;
      final int omset = laku * hargaJual;
